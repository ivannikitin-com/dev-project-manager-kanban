...
function cpm_url_ddd_index($project_id){
    $url = sprintf( '%s?page=cpm_projects&tab=ddd&action=index&pid=%d', admin_url( 'admin.php' ), $project_id );

    return apply_filters( 'cpm_url_ddd_index', $url, $project_id );
}

function cpm_url_single_ddd( $project_id, $list_id ) {
    $url = sprintf( '%s?page=cpm_projects&tab=ddd&action=index&pid=%d#/list/%d', admin_url( 'admin.php' ), $project_id);

    return apply_filters( 'cpm_url_single_ddd', $url, $project_id);
}
